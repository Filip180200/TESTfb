...
const checkConfigFileExist = () => {
  ...
  try {
    ...
    console.error(
      "Please provide a config-production.json or config-development.json file!",
      err,
    );
  }
};
...
const testConnection = async () => {
  ...
  try {
    ...
    await connection.query(
      `CREATE DATABASE IF NOT EXISTS \`${databaseConfig.name}\`;`,
    );
    ...
    console.error(
      "Exiting... Unable to check and/or create a database: ",
      error,
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (config && config instanceof Array) {
      for (let i = 0; i < config.length; i++) {
        ...
        if (data)
          console.log(
            `Skipping user with name ${config[i].username} as it already exist`,
          );
        else {
          const admin = {
            username: config[i].username,
    ...
    console.log(
      "Some error occured whicle creating admin credentials: ",
      error.message,
    );
...
try {
  ...
  console.log(
    "Trying to inserting entries for admin credentials in the database!",
  );
  ...
  var corsOptions = {
    origin:
      environment === "development"
        ...
        : ["https://52.29.206.207", "https://52.29.206.207"],
  };
